...
export function navigateTo(page) {
    ...
    if (pageEl) {
        ...
        pageEl.innerHTML = '<div style="display:flex;justify-content:center;padding:40px;">' + 
                          renderIcon('Loader2', { width: 32, height: 32 }) + '</div>';
...
export function toast(message, type = 'default', duration = 3000) {
    ...
    el.style.cssText = `
        padding: 12px 20px;
        background: ${type === 'error' ? '#FEE2E2' : type === 'success' ? '#E8F5E9' : 'white'};
        color: ${type === 'error' ? '#C62828' : type === 'success' ? '#2E7D32' : 'inherit'};
        border-left: 4px solid ${type === 'error' ? '#f44336' : type === 'success' ? '#4CAF50' : '#7B4F8E'};
        font-size: 0.9rem;
        min-width: 250px;
...
async function initApp(user) {
    ...
    document.getElementById('btn-logout').onclick = async () => {
        ...
        await logout();
        // Remove apenas as chaves do app, preserva outros dados do domínio
